...
# - Cron job on macOS to schedule the task to run daily
...
# - News API key: You can get an API key from [NewsAPI](https://newsapi.org/).
...
# To make sure the script runs at 8 AM every day:
# Open Terminal and run the following to set your Mac to wake up at a specific time:
# ```bash
...
# If you want to manually run the script, simply run:
# ```bash
# python3 main.py
# ```
